#!/usr/bin/env python3
"""
MOT Data Collection Spark Application - Functional Version

ใช้งานร่วมกับ Airflow DAG สำหรับดึงข้อมูล MOT API 
และอัพเดตเข้า PostgreSQL features table

Based on: mot_collecting_fucntional.py (working version)
...
def main():
    ...
    print("🚀 เริ่มต้นการอัปเดตข้อมูล MOT (Airflow Mode)")
    ...
    if len(sys.argv) >= 4:
        ...
        force_mode = sys.argv[4] if len(sys.argv) > 4 else None
    ...
    if latest_date is None:
        if not force_mode:
            ...
            return False
        # ถ้าเป็น force mode ให้เริ่มจากเดือนก่อน
        latest_date = date(2025, 8, 31)
        print(f"🔧 Force mode: กำหนดวันที่เริ่มต้น {latest_date}")
